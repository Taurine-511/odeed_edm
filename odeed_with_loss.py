...
def main(network_pkl, outdir, lpips_net, device=torch.device('cuda'), **sampler_kwargs):
    ...
    for batch_idx, (images, labels) in enumerate(dataset_iterator):
        with torch.no_grad():
            ...
            mse_loss = loss.calc_loss(images, generated_images, 'mse')
            lpips_loss = loss.calc_loss(images, generated_images, 'lpips')
            ...
            for idx, image_np in enumerate(images_np):
                ...
                loss_log['mse'].append((img_idx, mse_loss))
                loss_log['lpips'].append((img_idx, lpips_loss))

    # save loss log
    loss_log_path = os.path.join(image_dir, 'loss.json')
